...
class Boletas_model extends CI_Model {
	...
	function grafBoletasI(){
		$r = $this->db->query("
			SELECT count(*) as total, MONTH(inicio)as mes, YEAR(inicio)as ges
			FROM boleta
			WHERE YEAR(inicio)=YEAR(CURDATE())
			GROUP BY YEAR(inicio), MONTH(inicio)
			ORDER BY MONTH(inicio) ASC
		");
		return $r->result();
	}
	function grafBoletasF(){
		$r = $this->db->query("
			SELECT count(*) as total, MONTH(fin)as mes, YEAR(fin)as ges
			FROM boleta
			WHERE YEAR(fin)=YEAR(CURDATE())
			GROUP BY YEAR(fin), MONTH(fin)
			ORDER BY MONTH(fin) ASC
		");
		return $r->result();
	}
